...
void PgmStrCpy(char * dest, const char * src){
  char c;
  int i = 0;
  while ((c = pgm_read_byte_near(src++)))
    dest[i++]=(c);
  dest[c]=0;
}
...
#ifdef CUSTOM_VARIABLE_SPEED_CYCLE

uint16_t readADC(){
  PRR = PRR & (! bit(PRADC)); 
  const int channel = 4;
  ADMUX = bit(REFS0) | (channel & 0x07);
  ADCSRA = bit(ADSC) | bit(ADEN) | 0x07;
  ...
  // MC: For Niels. Home then go back and forth
    // Run Homing Cycle Manually
    ...
    while (serial_get_rx_buffer_count() == 0)
    {
      report_status_message(gc_execute_line(commands[i]));
      while(serial_get_rx_buffer_count() == 0 && plan_get_current_block() != NULL)
      {
        // Critical stuff to actually make movements happen
        protocol_auto_cycle_start();
        protocol_execute_realtime();

        // Update the feed rate based on a potentiometer connected to PC4 (A4).
        // Yes, this code is terrible, and has magic numbers, I just don't care anymore
        loop_count ++;
        if(loop_count == 20)
        {
          sum += (readADC());
        ...
        if (avg_count == 30)
        {
          uint8_t new_f_override = sum / avg_count / 10 ; // roughly from 0% to 102%
          //print_uint8_base10(new_f_override);
          //printPgmString(PSTR("\r\n"));
          sum = 0;
          // Do the feed override
